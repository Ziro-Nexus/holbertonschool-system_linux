...
int lto_str(PyObject *o, char **bytes_str);
char *dig_tostr(digit *pout, ssize_t size, int negative);
...
/**
 * lto_str - renders a python long int as a string
 * @o: pointer to python int
 * @str: address of char pointer which will be assiged
 * Return: -1 on error else 0
 */
int lto_str(PyObject *o, char **str)
{
	PyLongObject *a;
	...
	*str = dig_tostr(pout, size, negative);
	return (0);
}

/**
 * dig_tostr - renders vector of decimal digits to string
